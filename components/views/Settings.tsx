...
const Settings = ({ setView } : SettingProps) => {
  ...
  className="appearance-none border-b border-gray-700 bg-transparent text-white px-4 py-2 text-sm w-48 focus:outline-none focus:border-b-gray-300"
            /> */}
            <TimeInput
              value={watch("checkInTime")}
              onChange={(val) => setValue("checkInTime", val, { shouldValidate: true })}
            />
            {errors.checkInTime && (
          ...
          <div className='flex flex-col'>
            ...
            <TimeInput
              ...
              onChange={(val) => setValue("checkOutTime", val, { shouldValidate: true })}
            />
            {errors.checkOutTime && (
              <span className="text-red-500 text-xs mt-1">
